...
def get_top_products_dashboard(request):
    ...
    sum_filter = Q(product_sale_records__product_records__location=location_obj)
    products = Product.objects \
    .filter(
        product_stock__product_records__location_id = location_id,
        is_deleted = False,
        product_sale_records__product_records__location_id=location_id) \
    .annotate(
