...
class SingleNoteSerializer(serializers.ModelSerializer):
    ...
    client_total_appointment = serializers.SerializerMethodField(read_only=True)
    ...
    def get_client_total_appointment(self, obj):
        ...
        .order_by('-created_at')
        client_total_appointment = appointment_checkout_all.aggregate(total_sale=Sum('price')).get('total_sale', 0)
        return client_total_appointment
    
    class Meta:
        model = Appointment
        fields = ['id', 'client','client_name', 'client_email', 'client_phone', 'client_total_appointment','appointment_tips', 'notes', 'business_address',
