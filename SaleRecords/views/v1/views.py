            ...
            if serializer.is_valid():
                ...
                try :
                    ...
                    if loyalty_points_data is not None:
                        loyalty_points = ClientLoyaltyPoint.objects.get(id = loyalty_points_data.data.get('redeemed_id'))
                        # raise ValidationError('If part')
                        if loyalty_points:
                            client_points = ClientLoyaltyPoint.objects.get(id = loyalty_points.client_loyalty_point.id)
                            
                            client_points.points_redeemed = float(client_points.points_redeemed) + float(loyalty_points_data.data.get('redeemed_points'))
                            client_points.save()

                            single_point_value = client_points.customer_will_get_amount / client_points.for_every_points
                            total_redeened_value = float(single_point_value) * float(loyalty_points_data.data.get('redeemed_points'))

                            logs_points_redeemed = loyalty_points_data.data.get('redeemed_points')
